namespace AngleSharp.Html.Parser
{
        ...
        where TDocument : class, IConstructableDocument
    {
        ...
        public IConstructableElement? AdjustedCurrentNode =>
            (_fragmentContext != null && _openElements.Count == 1) ? _fragmentContext : CurrentNode;

        /// <summary>
        /// Gets the current node.
        /// </summary>
        public IConstructableElement CurrentNode => _openElements.Count > 0 ? _openElements[^1] : null!;
        ...
        /// The context element where the algorithm is applied to.
        /// </param>
        public HtmlDocument ParseFragment(HtmlParserOptions options, TElement context)
        {
            context = context ?? throw new ArgumentNullException(nameof(context));
        ...
        private void Consume(ref StructHtmlToken token)
        {
            ...
            if (node is null || token.Type == HtmlTokenType.EndOfFile ||
                ...
                ((node.Flags & NodeFlags.MathMember) == NodeFlags.MathMember && token.IsSvg &&
        ...
        private void InHead(ref StructHtmlToken token)
        {
            switch (token.Type)
            {
                ...
                case HtmlTokenType.StartTag:
                {
                    ...
                    {
                        var meta = _elementFactory.CreateMeta(_document);
                        AddElement(meta, ref token, acknowledgeSelfClosing: true);
                        CloseCurrentNode();
                    ...
                    {
                        // var script = new ReadOnlyHtmlScriptElement(_document, parserInserted: true, started: IsFragmentCase);
        ...
        private void InBodyStartTag(ref StructHtmlToken tag)
        {
            ...
            else if (tagName.Is(TagNames.A))
            {
                ...
                // var element = new ReadOnlyHtmlAnchorElement(_document);
            ...
            else if (TagNames._mAllHeadings.Contains(tagName))
            {
                ...
                if (TagNames._mAllHeadings.Contains(CurrentNode.LocalName))
                {
            ...
            {
                ReconstructFormatting();
                // var inputElement = new ReadOnlyHtmlInputElement(_document);
            ...
            else if (tagName.Is(TagNames.Form))
            {
                if (_currentFormElement is null)
                {
                    ...
                    // _currentFormElement = new ReadOnlyHtmlFormElement(_document);
            ...
            else if (tagName.Is(TagNames.Button))
            {
                ...
                {
                    ReconstructFormatting();
                    // AddElement(new ReadOnlyHtmlButtonElement(_document), ref tag);
            ...
            else if (tagName.Is(TagNames.Hr))
            {
                ...
                // AddElement(new ReadOnlyHtmlHrElement(_document), ref tag, acknowledgeSelfClosing: true);
            ...
            {
                //AddElement(new ReadOnlyHtmlTextAreaElement(_document), ref tag);
            ...
            {
                ReconstructFormatting();
                // AddElement(new ReadOnlyHtmlSelectElement(_document), ref tag);
            ...
            {
                var scripting = _options.IsScripting;
                // var element = new ReadOnlyHtmlNoScriptElement(_document, null, scripting);
                var element = _elementFactory.CreateNoScript(_document, scripting);
            ...
            {
                // var element = new ReadOnlyMathElement(_document, tagName);
                var element = _elementFactory.CreateMath(_document);
            ...
            {
                // var element = new ReadOnlySvgElement(_document, tagName);
                var element = _elementFactory.CreateSvg(_document);
            ...
            else if (tagName.Is(TagNames.Frameset))
            {
                ...
                if (_openElements.Count != 1 && _openElements[1].LocalName.Is(TagNames.Body) && _frameset)
                {
                    ...
                    // AddElement(new ReadOnlyHtmlFrameSetElement(_document), ref tag);
            ...
            else if (tagName.Is(TagNames.Body))
            {
                ...
                if (_templateModes.Count == 0 && _openElements.Count > 1 &&
                    _openElements[1].LocalName.Is(TagNames.Body))
                {
                    _frameset = false;
        ...
        private void InBodyEndTag(ref StructHtmlToken tag)
        {
            ...
            {
                if (IsInScope(TagNames._mAllHeadings))
                {
                    GenerateImpliedEndTags();
                    ...
                    ClearStackBackTo(TagNames._mAllHeadings);
                    CloseCurrentNode();
                }
                else
                {
            ...
            switch (token.Type)
            {
                ...
                case HtmlTokenType.StartTag:
                {
                    ...
                    if (tagName.Is(TagNames.Caption))
                    {
                        ...
                        _formattingElements.AddScopeMarker();
                        // AddElement(new ReadOnlyHtmlTableCaptionElement(_document), ref token);
                    ...
                    {
                        ClearStackBackTo(TagNames.Table);
                        // AddElement(new ReadOnlyHtmlTableColgroupElement(_document), ref token);
                    ...
                    {
                        ClearStackBackTo(TagNames.Table);
                        // AddElement(new ReadOnlyHtmlTableSectionElement(_document, tagName), ref token);
                    ...
                    else if (tagName.Is(TagNames.Input))
                    {
                        ...
                        if (tag.GetAttribute(AttributeNames.Type).Isi(AttributeNames.Hidden))
                        {
                            ...
                            // AddElement(new ReadOnlyHtmlInputElement(_document), ref tag, true);
                ...
                case HtmlTokenType.Character:
                {
                    if (TagNames._mAllTableMajor.Contains(CurrentNode.LocalName))
                    {
        ...
        private void InColumnGroup(ref StructHtmlToken token)
        {
            switch (token.Type)
            {
                ...
                case HtmlTokenType.StartTag:
                {
                    ...
                    {
                        // var col = new ReadOnlyHtmlTableColElement(_document);
        ...
        private void InTableBody(ref StructHtmlToken token)
        {
            switch (token.Type)
            {
                case HtmlTokenType.StartTag:
                {
                    ...
                    {
                        ClearStackBackTo(TagNames._mAllTableSections);
                        // var row = new ReadOnlyHtmlTableRowElement(_document);
        ...
        private void InCell(ref StructHtmlToken token)
        {
            switch (token.Type)
            {
                case HtmlTokenType.StartTag:
                {
                    ...
                    {
                        if (IsInTableScope(TagNames._mAllTableCells))
                        {
            ...
            switch (token.Type)
            {
                ...
                case HtmlTokenType.StartTag:
                {
                    ...
                    else if (tagName.Is(TagNames.Option))
                    {
                        ...
                        // AddElement(new ReadOnlyHtmlOptionElement(_document), ref token);
                    ...
                    else if (tagName.Is(TagNames.Optgroup))
                    {
                        ...
                        // AddElement(new ReadOnlyHtmlOptionsGroupElement(_document), ref token);
        ...
        private void InFrameset(ref StructHtmlToken token)
        {
            switch (token.Type)
            {
                ...
                case HtmlTokenType.StartTag:
                {
                    ...
                    {
                        // AddElement(new ReadOnlyHtmlFrameSetElement(_document), ref token);
                        var frameset = _elementFactory.Create(_document, TagNames.Frameset);
        ...
        {
            if (IsInTableScope(TagNames._mAllTableSections))
            {
                ClearStackBackTo(TagNames._mAllTableSections);
                CloseCurrentNode();
                _currentMode = HtmlTreeMode.InTable;
        ...
        {
            // AddElement(new ReadOnlyHtmlBodyElement(_document), ref token);
        ...
        private void InBodyStartTagListItem(ref StructHtmlToken tag)
        {
            ...
            while (true)
            {
                ...
                if (((node.Flags & NodeFlags.Special) == NodeFlags.Special) &&
                    !TagNames._mAllBasicBlocks.Contains(node.LocalName))
                {
                    break;
                }
        ...
        private void HeisenbergAlgorithm(ref StructHtmlToken tag)
        {
            ...
            if (currentNode.NamespaceUri.Is(NamespaceNames.HtmlUri) &&
                currentNode.LocalName.Is(tag.Name) &&
                !_formattingElements.Contains(currentNode))
            {
            ...
            for (var outer = 0; outer < 8; outer++)
            {
                ...
                if (!TagNames._mAllTableMajor.Contains(commonAncestor.LocalName))
                {
        ...
        private void InCellEndTagCell(ref StructHtmlToken token)
        {
            ...
            if (!TagNames._mAllTableCells.Contains(CurrentNode.LocalName))
            {
                RaiseErrorOccurred(HtmlParseError.TagDoesNotMatchCurrentNode, ref token);
            }

            ClearStackBackTo(TagNames._mAllTableCells);
            CloseCurrentNode();
            _formattingElements.ClearFormatting();
            _currentMode = HtmlTreeMode.InRow;
        }
            ...
            switch (token.Type)
            {
                ...
                case HtmlTokenType.StartTag:
                {
                    ...
                    if (tagName.Is(TagNames.Font))
                    {
                        ...
                        {
                            if (token.Attributes[i].Name.IsOneOf(AttributeNames.Color, AttributeNames.Face,
                                    AttributeNames.Size))
                            {
        ...
        /// </summary>
        /// <returns>True if it is in scope, otherwise false.</returns>
        private Boolean IsInScope(HashSet<ReadOnlyMemory<Char>> tags)
        {
            for (var i = _openElements.Count - 1; i >= 0; i--)
            {
                var node = _openElements[i];
        ...
        /// </summary>
        /// <returns>True if it is in scope, otherwise false.</returns>
        private Boolean IsInTableScope(HashSet<ReadOnlyMemory<Char>> tags)
        {
            for (var i = _openElements.Count - 1; i >= 0; i--)
            {
                var node = _openElements[i];
        ...
        /// </summary>
        private Boolean IsCustomElementEverywhere(StringOrMemory tagName) =>
            _options.IsAcceptingCustomElementsEverywhere && tagName.IsCustomElement();
        ...
        private void CloseCurrentNode()
        {
            if (_openElements.Count > 0)
            {
                ...
                var node = AdjustedCurrentNode;
                _tokenizer.IsAcceptingCharacterData =
                    node is not null && ((node.Flags & NodeFlags.HtmlMember) != NodeFlags.HtmlMember);
            }
        }
        ...
        private void AddElement(IConstructableElement element)
        {
            ...
            if (_foster && TagNames._mAllTableMajor.Contains(node.LocalName))
            {
        ...
        /// </summary>
        private void ClearStackBackTo(HashSet<ReadOnlyMemory<Char>> tags)
        {
            var node = CurrentNode;
