...
A `MutationObserver` reports *what* changed, allocates a record per mutation and delivers asynchronously. When the question is only *whether* anything changed - invalidating a cache over a live collection, for instance - `Document.MutationVersion` answers it without any of that. It is advanced synchronously with the mutation itself, costs a single increment, and covers every scripted and programmatic DOM write, so two equal readings mean nothing in the document changed in between. Compare two readings for equality; the value itself is opaque. It is declared on `Document` rather than on `IDocument`, so reaching it means a cast: `((Document)document).MutationVersion`.

Parsing does not advance it. The tree builder constructs the tree through raw child list, attribute and character data operations that run no mutation logic at all, so a freshly parsed document still reads the value it started at - which is also what keeps the counter free on the parse path. A cache keyed on the version is therefore valid across two equal readings of one parsed document, but a parse boundary is an invalidation point of its own: that includes `document.open`, which discards the tree without notification, and anything a script writes with `document.write`, which goes to the tokenizer rather than to the DOM.
