...
namespace AngleSharp.Html
{
    ...
    class TreeConstruction
    {
        ...
        /// <summary>
        /// The event will be fired once the parsing should immediately end.
        /// </summary>
        ...
        /// <summary>
        /// Creates a new instance of the tree constructor
        /// with the specified document and tokenizer.
        /// </summary>
        ...
        /// </summary>
        public void Reset(Node context = null)
        {
            var last = false;
            Node node;
        ...
        public void Consume(HtmlToken token)
        {
            if (skipNext)
            {
                skipNext = false;

                if (token.Type == HtmlTokenType.Character && ((HtmlCharacterToken)token).Data == Specification.LF)
                    return;
            }
            
            if ((CurrentNode == null) ||
                   (CurrentNode.IsInHtml) ||
                   (CurrentNode.IsInMathML && CurrentNode.NodeName == "annotation-xml" && token.Type == HtmlTokenType.StartTag && ((HtmlTagToken)token).Name == "svg") ||
                   (CurrentNode.IsMathMLTIP && (token.Type == HtmlTokenType.Character || (token.Type == HtmlTokenType.StartTag && (((HtmlTagToken)token).Name != "mglyph" && ((HtmlTagToken)token).Name != "malignmark")))) ||
                   (CurrentNode.IsHtmlTIP && (token.Type == HtmlTokenType.StartTag || token.Type == HtmlTokenType.Character)) ||
                   (token.Type == HtmlTokenType.EOF))
            {
                switch (insert)
                {
        ...
        void BeforeHtml(HtmlToken token)
        {
            ...
            {
                var element = new HTMLHtmlElement();
                AddElementToDocument(element, HtmlTagToken.Open.SetName(HTMLHtmlElement.Tag));
                //TODO
                //If the Document is being loaded as part of navigation of a browsing context, then:
        ...
        void BeforeHead(HtmlToken token)
        {
            ...
            {
                BeforeHead(HtmlTagToken.Open.SetName(HTMLHeadElement.Tag));
                InHead(token);
            }
        }
        ...
        void InHead(HtmlToken token)
        {
            ...
            else if (token.Type == HtmlTokenType.StartTag && ((HtmlTagToken)token).Name == HTMLMetaElement.Tag)
            {
                ...
                if (charset != null && charset.Equals("Content-Type", StringComparison.InvariantCultureIgnoreCase))
                {
            ...
            else if (token.Type == HtmlTokenType.StartTag && ((HtmlTagToken)token).Name == HTMLScriptElement.Tag)
            {
                ...
                AddElementToCurrentNode(element, token);
                tokenizer.Switch(ContentModel.Script);
        ...
        void AfterHead(HtmlToken token)
        {
            ...
            {
                AfterHeadStartTagBody(HtmlTagToken.Open.SetName(HTMLBodyElement.Tag));
                frameset = true;
                Consume(token);
            }
        }
        ...
        void InBody(HtmlToken token)
        {
                    ...
                    case HTMLSemanticElement.PlaintextTag:
                        {
                            ...
                            tokenizer.Switch(ContentModel.Plaintext);
                        }
                        break;
                    ...
                    case HTMLInputElement.Tag:
                        {
                            ...
                            if (!tag.GetAttribute("type").Equals("hidden", StringComparison.InvariantCultureIgnoreCase))
                                frameset = false;
                        }
                        break;
                    ...
                    case HTMLIsIndexElement.Tag:
                        {
                            ...
                            if (form == null)
                            {
                                InBody(HtmlTagToken.Close.SetName(HTMLFormElement.Tag));

                                if (tag.GetAttribute("action") != string.Empty)
                                    form.SetAttribute("action", tag.GetAttribute("action"));

                                InBody(HtmlTagToken.Close.SetName(HTMLHRElement.Tag));
                                InBody(HtmlTagToken.Close.SetName(HTMLLabelElement.Tag));
                                ...
                                var input = HtmlTagToken.Close.SetName(HTMLInputElement.Tag);
                                ...
                                InBody(input);
                                InBody(HtmlTagToken.Open.SetName(HTMLLabelElement.Tag));
                                InBody(HtmlTagToken.Close.SetName(HTMLHRElement.Tag));
                                InBody(HtmlTagToken.Open.SetName(HTMLFormElement.Tag));
                            }
                        }
                        break;
                    ...
                    case HTMLTextAreaElement.Tag:
                        {
                            ...
                            AddElementToCurrentNode(element, token);
                            skipNext = true;
                            tokenizer.Switch(ContentModel.RCData);
                            originalInsert = insert;
                            frameset = false;
                            insert = InsertationMode.Text;
                        }
                    ...
                    case HTMLOptionElement.Tag:
                        {
                            if (CurrentNode.NodeName == HTMLOptionElement.Tag)
                                InBodyEndTagAnythingElse(HtmlTagToken.Close.SetName(HTMLOptionElement.Tag));
            ...
            else if (token.Type == HtmlTokenType.EndTag)
            {
                ...
                switch (tag.Name)
                {
                    ...
                    case HTMLBRElement.Tag:
                        RaiseErrorOccurred(ErrorCode.TagCannotEndHere);
                        InBodyStartTagBreakrow(HtmlTagToken.Open.SetName(HTMLBRElement.Tag));
                        break;
        ...
        void InTable(HtmlToken token)
        {
            ...
            else if (token.Type == HtmlTokenType.StartTag)
            {
                ...
                switch (tag.Name)
                {
                    ...
                    case HTMLTableColElement.ColTag:
                        {
                            InTable(HtmlTagToken.Open.SetName(HTMLTableColElement.ColgroupTag));
                            InColumnGroup(token);
                        }
                        break;
                    ...
                    case HTMLTableRowElement.Tag:
                        {
                            InTable(HtmlTagToken.Open.SetName(HTMLTableSectionElement.BodyTag));
                            InTableBody(token);
                        }
                        break;
        ...
        void InTableText(HtmlToken token)
        {
            ...
            else
            {
                ...
                {
                    for (var i = 0; i != tableCharacters.Length; i++)
                        InBodyWithFoster(new HtmlCharacterToken(tableCharacters[i]));
                }
                else
                {
                    for (var i = 0; i != tableCharacters.Length; i++)
        ...
        void InTableBody(HtmlToken token)
        {
            if (token.Type == HtmlTokenType.StartTag)
            {
                ...
                {
                    InTableBody(HtmlTagToken.Close.SetName(HTMLTableRowElement.Tag));
                    InRow(token);
                }
        ...
        void RawtextAlgorithm(HtmlTagToken tag)
        {
            ...
            originalInsert = insert;
            insert = InsertationMode.Text;
            tokenizer.Switch(ContentModel.Rawtext);
        }
        ...
        void InBodyStartTagListItem(HtmlTagToken tag)
        {
            ...
            while (true)
            {
                ...
                {
                    InBody(HtmlTagToken.Close.SetName(node.NodeName));
                    break;
                }
        ...
        bool InBodyEndTagParagraph()
        {
            ...
            else
            {
                ...
                InBody(HtmlTagToken.Open.SetName(HTMLParagraphElement.Tag));
                InBodyEndTagParagraph();
                return false;
            }
        }
                    ...
                    default:
                        {
                            ...
                            if (!tag.IsSelfClosing)
                                ...
                                Foreign(HtmlTagToken.Close.SetName(HTMLScriptElement.Tag));
                        }
        ...
        /// <summary>
        /// Inserts the given character token into the current node.
        /// </summary>
        /// <param name="token">The character token to insert.</param>
        void InsertCharacter(HtmlToken token)
        {
            var ct = (HtmlCharacterToken)token;
            InsertCharacter(ct.Data);
        }
        ...
        void ReconstructFormatting()
        {
            ...
            for (; index < formatting.Count; index++)
            {
                ...
                AddElementToCurrentNode(element, HtmlTagToken.Open.SetName(entry.NodeName));
        ...
        /// <summary>
        /// Fires once the current node has changed.
        /// </summary>
        ...
        /// <summary>
        /// Raises the stop parsing event.
        /// </summary>
        void RaiseStopParsing()
        {
            End();
