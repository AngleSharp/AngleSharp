namespace AngleSharp.DOM.Css
{
    ...
    sealed class CSSBackgroundRepeatProperty : CSSProperty, ICssBackgroundRepeatProperty
    {
        ...
        internal static readonly IValueConverter<Repeat> SingleConverter = From(Map.BackgroundRepeats).To(m => new Repeat { Horizontal = m, Vertical = m }).Or(
                   TakeOne(Keywords.RepeatX, RepeatX)).Or(TakeOne(Keywords.RepeatY, RepeatY)).Or(WithArgs(From(Map.BackgroundRepeats).Required(), From(Map.BackgroundRepeats).Required(), m => new Repeat { Horizontal = m.Item1, Vertical = m.Item2 }));
